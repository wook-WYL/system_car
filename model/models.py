...
class Baseline(nn.Module):
    ...
    def __init__(self, guide, block=BasicBlock, bc=16, \
                 ...
                 norm_layer=nn.BatchNorm2d, weight_ks=3, kernel_sizes=[3, 5, 7]):
        super().__init__()

        self._norm_layer = norm_layer
        in_channels = bc * 2
        # 打印调试信息
        print(f"Initializing Baseline with norm_layer: {norm_layer}, type: {type(norm_layer)}")
        print(f"Guide type: {guide}, type: {type(guide)}")
        ...
        # print(f"guide1 initialized with norm_layer: {norm_layer}, type: {type(norm_layer)}")
        ...
        self.guide1 = guide(in_channels * 2, in_channels * 2, norm_layer, kernel_sizes)
        self.guide2 = guide(in_channels * 4, in_channels * 4, norm_layer, kernel_sizes)
        self.guide3 = guide(in_channels * 8, in_channels * 8, norm_layer, kernel_sizes)
        self.guide4 = guide(in_channels * 8, in_channels * 8, norm_layer, kernel_sizes)
