...
class RELLIS_3D(data.Dataset):
    ...
    def make_sample_dataset(self):
        ...
        for seq in self.seqs:
            ...
            file_list = file_list[self.frame_crop_idx[seq][0]:self.frame_crop_idx[seq][
                1]]  # first / last frames don't have footprints..

            for fn in file_list:
                str_frame = fn[:-4]
                if self.mode != "test":
                    indiv_sample = {"image": os.path.join(self.data_path, "Rellis-3D", seq_str, "pylon_camera_node",
                                                          cam_name_dict[seq][str_frame] + ".jpg"),
                                    "point": os.path.join(self.data_path, "Rellis-3D", seq_str,
                                                          "os1_cloud_node_kitti_bin", str_frame + '.bin'),
                                    "sn": os.path.join(self.data_path, "Rellis-3D-custom", seq_str, "surface_normal",
                                                       str_frame + '.npy'),
                                    "sp": os.path.join(self.data_path, "Rellis-3D-custom", seq_str, "super_pixel",
                                                       str_frame + ".png"),
                                    "fp": os.path.join(self.data_path, "Rellis-3D-custom", seq_str, "foot_print",
                                                       str_frame + ".png"),
    ...
    def __getitem__(self, index):
        ...
        im_np = resize_rgb_image(im_np, (
        int(self.raw_cam_img_size[0] / self.ratio), int(self.raw_cam_img_size[1] / self.ratio)))
        ...
        depth_np = resize_depth_image(depth_np, (
        int(self.raw_cam_img_size[0] / self.ratio), int(self.raw_cam_img_size[1] / self.ratio)))
