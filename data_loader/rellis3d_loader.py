...
class RELLIS_3D(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        im_np = im_np[:, -self.input_img_size[0]:, -self.input_img_size[1]:]
        depth_np = depth_np[:, -self.input_img_size[0]:, -self.input_img_size[1]:]
        ...
        if self.mode != "test":
            ...
            if random.random() < 0.5:
                im_np = image_flip(im_np)
                depth_np = image_flip(depth_np)
        ...
        return im_np,depth_np, gts, self.samples[index]["fname"]
