...
def compute_score(slope, height, roughness, s_crit=5, h_crit=1, r_crit=0.005, omega1=1, omega2=0.3, omega3=0):
    ...
    """
    # 调整权重，注重坡度和粗糙度
    score = omega3 * (1 / (1 + np.exp(-10 * (slope - s_crit)))) + \
            omega1 * (1 / (1 + np.exp(-1 * (height - h_crit)))) + \
            omega2 * (1 / (1 + np.exp(-50 * (roughness - r_crit))))
...
if __name__ == "__main__":
    ...
    for i, str_frame in enumerate(tqdm(file_list, desc=f"[i] Processing images from {data_path}")):
        ...
        for j, point_2d in enumerate(pts_2d):
            ...
            if 0 <= x < image.shape[1] and image.shape[0]/3 <= y < image.shape[0]:
                ...
                color = (0, 255, 0) if score_value <= 0.6 else (0, 0, 255)
