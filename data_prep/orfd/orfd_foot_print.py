...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_pose", type=int, help="", default=1000)
    ...
    save_root = os.path.join(args.data_root, "ORFD-custom", args.mode, "foot_print3")
    ...
    for seq_name in seq_dict.keys():
        ...
        if not os.path.isfile(
            os.path.join(args.data_root, "ORFD-custom", args.mode, "pose", "pose_" + seq_name + ".csv")): continue
        ...
        for i, str_frame in enumerate(tqdm(seq_dict[seq_name], desc=description)):
            ...
            calib_mtx = read_calib_file_orfd(
                os.path.join(args.data_root, "Final_Dataset", args.mode,"c2021_0228_1819", 'calib', str_frame + '.txt'))
            image = io.imread(
                os.path.join(args.data_root, "Final_Dataset", args.mode, "c2021_0228_1819", "image_data", str_frame + '.png'))
            pc = bin_read(os.path.join(args.data_root, "Final_Dataset", args.mode,  "c2021_0228_1819", 'lidar_data', str_frame +'.bin'))
