...
def process_sequence(data_root, mode, radius, k):
    """处理流程增强版"""
    save_root = os.path.join(data_root, "ORFD-custom", mode, "roughness")
    os.makedirs(save_root, exist_ok=True)

    bin_path = os.path.join(data_root, "Final_Dataset", mode, "lidar_data")
    file_list = natsort.natsorted(os.listdir(bin_path))

    for fn in tqdm(file_list, desc=f"Processing Seq {mode}", unit="file"):
        save_file = os.path.join(save_root, fn[:-4] + "_roughness.npy")

        # 如果文件已经存在并且完整，则跳过
        if os.path.exists(save_file):
            tqdm.write(f"Skipping {fn}, file already processed.")
            continue
        ...
        # 确保数据长度是5的倍数
        if len(pc) % 5 != 0:
            tqdm.write(f"Warning: {fn} contains unexpected data size, skipping this file.")
            continue

        # 重塑数据并保留前4个字段（x, y, z, intensity）
        pc = pc.reshape(-1, 5)[:, :4]  # 仅保留前4个字段（坐标 + 反射强度）

        # 计算粗糙度
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(pc[:, :3])  # 只使用前三列作为坐标
        pcd_tree = o3d.geometry.KDTreeFlann(pcd)
        roughness, valid_count = compute_roughness(pc[:, :3], pcd_tree, radius, k)
...
if __name__ == "__main__":
    ...
    process_sequence(args.data_root, args.mode, args.radius, args.k)

    print("\n=== 处理完成 ===")
    # python roughness.py --data_root E:\BaiduNetdiskDownload\ORFD_Dataset_ICRA2022_ZIP --mode training --radius 0.3  --k 30
